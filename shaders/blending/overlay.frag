...
float _overlay(float dst, float src) {
    return dst < 0.5 ? (2.0*dst*src) : (1.0-2.0*(1.0-dst)*(1.0-src));
}

vec3 blend(vec3 dst, vec4 src) {
    float _opacity = (src.a*opacity);
    return vec3(_overlay(dst.r, src.r), _overlay(dst.g, src.g), _overlay(dst.b, src.b)) * _opacity + dst * (1.0-_opacity);
}
